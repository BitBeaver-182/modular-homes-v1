...
| 2026-06-01 | Frontend Form Patterns | Replacing established dashboard form structure with ad hoc modal/form implementations in new features | Follow the existing feature pattern used by `quotes`: keep form fields in dedicated field components/files, keep dialogs thin, and preserve the same field composition approach unless the user explicitly approves a new pattern. | v1.0 |
| 2026-06-01 | Server Validation UX | Catching mutation errors locally, showing generic toasts, or otherwise preventing backend validation errors from marking the actual form fields | Let the mutation error propagate back into the form helper, map backend field paths onto RHF fields, and verify the field invalid state and field error rendering match the `quotes` example before considering the form done. [REVISION - 2026-06-01: server validation is the default for supplier-order product and invoice forms; do not rely on client-side completeness toasts for required-field validation.] | v1.1 |
...
- **Log Entry #10 (Frontend Form Structure + Validation Discipline):** Added rules to keep new dashboard forms aligned with the `quotes` structure and to let backend validation drive field-level error states instead of generic toasts.
...
- **Log Entry #14 (Quote Form Error Discipline):** Added a rule to match the explicit per-field error display pattern used by quote forms.
- **Log Entry #15 (Frontend Structure Discipline):** Added a rule to split complex feature UIs into focused files instead of monolithic components.
