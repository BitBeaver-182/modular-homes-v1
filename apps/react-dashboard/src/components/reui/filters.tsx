...
export interface CustomRendererProps<T = unknown> {
	...
	operator: string;
	/** Matches the parent `Filters` `size` — use on value triggers to align with select fields. */
	filterSize?: "sm" | "default" | "lg";
}
...
const flattenFields = <T = unknown,>(
	...
	return fields.reduce<Array<FilterFieldConfig<T>>>((accumulator, item) => {
		if (isFieldGroup(item)) {
			return [...accumulator, ...item.fields];
		}
								...
								{filteredUnselectedOptions.map((option, index) => {
									...
									return (
										<DropdownMenuCheckboxItem
											...
											onMouseEnter={() => {
												setHighlightedIndex(overallIndex);
											}}
											onCheckedChange={() => {
	...
	if (field.type === "text") {
		return (
			<FilterInput
				...
				onChange={(e) => {
					onChange([e.target.value] as Array<T>);
				}}
			/>
		);
	}
...
interface FiltersProps<T = unknown> {
	...
	showSearchInput?: boolean;
	trigger?: React.ReactNode;
	/** Show/hide add-filter trigger row. */
	showTrigger?: boolean;
	/** Show/hide active chips row. */
	showChips?: boolean;
	allowMultiple?: boolean;
	menuPopupClassName?: string;
	...
	/** Default `inline`: add menu and chips share one flex-wrap row. `stacked`: toolbar row (leading + add + trailing), then chips row. */
	layout?: "inline" | "stacked";
	toolbarLeading?: React.ReactNode;
	toolbarTrailing?: React.ReactNode;
	/** Optional content shown at the end of the chips row when `layout="stacked"` (e.g. "Clear all"). */
	chipsTrailing?: React.ReactNode;
	/** In stacked mode, render chips row before toolbar row. */
	chipsFirst?: boolean;
}
...
export function Filters<T = unknown>({
	...
	const chipList = filters.map((filter) => {
		const field = fieldsMap[filter.field];
		if (!field) {
			return null;
		}
	...
	const addFilterDropdown =
		showTrigger && selectableFields.length > 0 ? (
			<DropdownMenu
				open={addFilterOpen}
				onOpenChange={(open) => {
									...
									onKeyDown={(e) => {
										if (e.key === "ArrowDown") {
											...
											if (filteredFields.length > 0) {
												setHighlightedIndex((previous) =>
													previous < filteredFields.length - 1
														? previous + 1
														: 0
												);
											}
	...
	return (
		...
		>
			{layout === "stacked" ? (
						...
						<>
							{showChips ? (
								...
								>
									<div
										className={cn(
											"flex min-w-0 w-full flex-wrap items-center",
											size === "sm" && "gap-1.5",
							...
							{showTrigger || toolbarLeading || toolbarTrailing ? (
								<div
									className={cn(
				...
				>
					{showTrigger ? addFilterDropdown : null}
					{showChips ? chipList : null}
				</div>
			)}
		</FilterContext.Provider>
	);
