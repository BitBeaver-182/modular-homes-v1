...
const DataGridTableFillHeadCell = () => {
	const { props } = useDataGrid();
...
const DataGridTableFillBodyCell = () => {
	const { props } = useDataGrid();
...
const DataGridTableFillFootCell = () => {
	const { props } = useDataGrid();
		...
		>
			<colgroup>
				{visibleColumns.map((column) => {
					const isFlex = column.columnDef.meta?.flex === true;
						...
						<col
							...
							style={
								props.tableLayout?.columnsResizable
									...
									: props.tableLayout?.width === "fixed" && !isFlex
										? { width: column.getSize() }
										: undefined
							}
						/>
...
const DataGridTableResizeIndicator = ({
	viewportElement,
}: {
...
const DataGridTableBodyRowSkeleton = ({
	children,
}: {
	children: ReactNode;
}) => {
				...
				{Array.from({ length: pagination.pageSize }).map((_, rowIndex) => (
					<DataGridTableBodyRowSkeleton key={rowIndex}>
						{table.getVisibleFlatColumns().map((column, colIndex) => (
							<DataGridTableBodyRowSkeletonCell key={colIndex} column={column}>
...
const MemoizedDataGridTableBodyRows = memo(
	...
	(_previous, next) => !!next.table.getState().columnSizingInfo.isResizingColumn
				...
				<DataGridTableHead>
					{table
						...
						.map((headerGroup: HeaderGroup<TData>, index) => {
							return (
								<DataGridTableHeadRow key={index} headerGroup={headerGroup}>
									{headerGroup.headers.map((header, index) => {
										const { column } = header;

										return (
											<DataGridTableHeadRowCell key={index} header={header}>
