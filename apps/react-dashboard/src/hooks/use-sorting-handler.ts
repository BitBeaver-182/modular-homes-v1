...
interface SortingParams<TSortBy extends string = string> {
	sortBy?: TSortBy;
	sortOrder?: "asc" | "desc";
}
...
interface UseSortingHandlerOptions<
...
interface UseSortingHandlerReturn {
...
>({
	...
	useEffect(() => {
		if (didApplyDefaultSortRef.current) {
			return;
		}
