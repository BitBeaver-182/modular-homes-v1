...
function truncateToDecimalDigits(value: number, decimalDigits: number): number {
	...
	const adjusted =
		scaled >= 0 ? scaled + Number.EPSILON : scaled - Number.EPSILON;
...
export const useCurrency = (): UseCurrencyReturn => {
	...
	const getCurrencies = useCallback(
		(code: string): ReturnType<typeof CurrencyList.get> => {
			return CurrencyList.get(code, locale);
		},
		[locale]
	);

	const getDecimalDigits = useCallback(
		(code: string): number => {
	...
	return useMemo(
		...
		[getDecimalDigits, formatAmount, getCurrencies]
	);
};
